...
static int account_event(r_device* device, bitbuffer_t* bits, char const* demod_name) {
  return account_event_with_copy(device, bits, NULL, NULL, demod_name);
}
...
int pulse_slicer_manchester_zerobit_with_copy(pulse_data_t const* pulses, r_device* device, u_int8_t* dataBuffer, int* pBufferSize) {
  float samples_per_us = pulses->sample_rate / 1.0e6;
  ...
  for (unsigned n = 0; n < pulses->num_pulses; ++n) {
    ...
    if (((n == pulses->num_pulses - 1) // No more pulses? (FSK)
         || (pulses->gap[n] > s_reset)) // Long silence (OOK)
        && (bits.num_rows > 0)) { // Only if data has been accumulated
      events += account_event_with_copy(device, &bits, dataBuffer, pBufferSize, __func__);
      bitbuffer_clear(&bits);
      bitbuffer_add_bit(&bits, 0); // Prepare for new message with hardcoded 0
      time_since_last = 0;
    }
...
int pulse_slicer_manchester_zerobit(pulse_data_t const* pulses, r_device* device) {
  return pulse_slicer_manchester_zerobit_with_copy(pulses, device, NULL, NULL);
}
