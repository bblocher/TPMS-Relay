...
void rtl_433_ESP::setCallback(rtl_433_ESPCallBack callback, char* messageBuffer,
                              int bufferSize, uint8_t* dataBuffer, int dataBufferSize) {
  // logprintfLn(LOG_DEBUG, "rtl_433_ESP::setCallback location: %p", callback);
  _callback = callback;
  _messageBuffer = messageBuffer;
  _bufferSize = bufferSize;
  _setCallback(callback, messageBuffer, bufferSize, dataBuffer, dataBufferSize);
}
...
void rtl_433_ESP::getStatus() {
  ...
  (_callback)(_messageBuffer, NULL, 0);
  data_free(data);
}
